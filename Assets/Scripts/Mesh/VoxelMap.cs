...
public class VoxelMap : MonoBehaviour
{
    ...
    public void EstablishVoxels(int mapSize, GameObject mapPlane, float[,] noiseMap, TerrainType[] regions, bool cubeHeight)
    {
        ...
        Vector3 voxelSize = new Vector3((mapPlane.transform.localScale.x * 10) / mapSize, 2.0f, (mapPlane.transform.localScale.z * 10) / mapSize); // Set the desired voxel size
        ...
        for (int x = 0; x < mapSize; x++)
        {
            for (int z = 0; z < mapSize; z++)
            {
                ...
                voxels[index] = new GameObject();
                MeshFilter voxelMeshFilter = voxels[index].AddComponent<MeshFilter>();
                voxelMeshFilter.mesh = Resources.GetBuiltinResource<Mesh>("Cube.fbx");
                MeshRenderer voxelMeshRenderer = voxels[index].AddComponent<MeshRenderer>();
                voxelMeshRenderer.material = voxelMat;
                

                // Establish individual voxel position
                originPosition = transform.position - new Vector3((float)15 - ((float)voxelSize.x / 2), 0, (float)15 - ((float)voxelSize.z / 2)); // Set to this instance of voxel
                ...
                for (int i = 0; i < regions.Length; i++)
                {
                    if (currentHeight <= regions[i].height)
                    {
                        ...
                        if (cubeHeight)
                        {
                            originPosition.y = Mathf.RoundToInt(noiseMap[x, z] * (regions[i].height * 5) - 0.5f);
                        }
                        else
                        {
